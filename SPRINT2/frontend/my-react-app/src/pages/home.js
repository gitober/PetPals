...
useEffect(() => {
  ...
  simulateTestMode(`Toggling like for image URL: ${imageUrl}`);

  setLikeCounts((prevLikeCounts) => {
    const updatedLikeCounts = { ...prevLikeCounts };
    if (updatedLikeCounts[imageUrl]) {
      updatedLikeCounts[imageUrl] = 0;
    } else {
      updatedLikeCounts[imageUrl] = 1;
    }
...
const handleFileChange = (event) => {
  ...
  if (files.length > 0) {
    ...
    if (isTestMode) {
      ...
      const imageUrls = newImages.map((image) => URL.createObjectURL(new Blob([image])));
    ...
    <Layout>
      ...
      >
        ...
        <div className="comment-popup-content">
          ...
          {feedItems
        .filter((item) => item.images && item.images.includes(selectedImages[0]))
        .map((item, index) => (
